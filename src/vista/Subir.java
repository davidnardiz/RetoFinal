...
public class Subir extends JDialog implements ActionListener {
	...
	private JComboBox<String> etiquetables;
	...
	public Subir(ParaTi paraTi, boolean b, DAO dao, Usuario usu) {
		...
		setLocationRelativeTo(null);
				...
				segundos.setBounds(511, 445, 62, 22);
				contentPanel.add(segundos);
				segundos.setVisible(false);
		
		List<Usuario> etiquetablesList = dao.listarUsuario();		
		
		etiquetables = new JComboBox<String>();
		etiquetables.setSelectedIndex(-1);
		etiquetables.setBounds(238, 466, 300, 34);
		contentPanel.add(etiquetables);
		
		for (int i = 0; i < etiquetablesList.size(); i++) {
			etiquetables.addItem(etiquetablesList.get(i).getUsuario());
		}
	...
	private void subirFoto() {
		...
		if (comprobarCampos()) {
			...
			if (etiquetables.getSelectedIndex() != -1) {
				publi.setEtiquetado(etiquetables.getSelectedItem().toString());
			}
	...
	private boolean comprobarCampos() {
		...
		etiquetables.setBackground(new Color(0, 153, 51));
	...
	private void limpiar() {
		JComboBox[] comboBox = { cancion, resolucion, tipoHistoria, etiquetables };
	...
	private void historia() {
		...
		JComponent[] ocultar = { resolucion, lblResolucion, etiquetables, lblEtiquetado, duracion, lblDuracion, segundos,
	...
	private void reel() {
		...
		JComponent[] ocultar = { resolucion, lblResolucion, etiquetables, lblEtiquetado, rdbtnSi, rdbtnNo,
	...
	private void imagen() {
		this.limpiar();
		JComponent[] mostrar = { resolucion, lblResolucion, etiquetables, lblEtiquetado, lblDescripcion, descripcion };
